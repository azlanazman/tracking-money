...
| I.2 | Health Score explainer — expandable breakdown with guidance | ⬜ Not started | `index.html` (paste `ins-budgets` div) + `js/app.js` (paste `renderBudgets` function + `calculateHealthScore` function) | "In index.html, make the Financial Health score card in `ins-budgets` expandable. Add a `<button id='hs-expand-btn'>Full Record ↓</button>` below the 2×2 sub-signal grid (14px, `color:var(--ink-3)`, no border, background transparent, cursor pointer). Clicking it toggles a `<div id='hs-explainer'>` panel (`display:none` by default, `border-top:1px solid var(--line); margin-top:16px; padding-top:16px`). The panel contains: (1) a score change line: 'Score this period: {score}. Change from last period: {delta} pts.' — compute last period score by running `calculateHealthScore()` on filtered `txs` from `PAY_PERIOD.lastNPeriods(txs, 1)[0]`; show `+N` in `var(--accent)` or `-N` in `var(--warn)`. (2) For each of the 4 sub-signals, a `<div>` row with: signal name bold, score, descriptor (already returned from `calculateHealthScore`), and a 'How to improve' tip (hardcoded per signal: budget adherence → 'Spend RM {(totalBudget - totalSpent) / Math.max(daysLeft,1) | 0} or less per day to finish on track.'; savings rate → 'Transfer RM {Math.round(incAmt * 0.2 - savAmt)} more this period to hit 20%.'; runway → 'You have {daysLeft} days to payday — average RM {Math.round((totalBudget-totalSpent)/Math.max(daysLeft,1))} remaining per day.'; spend variance → 'Try to keep daily spending within RM {Math.round(meanDailySpend * 1.3)} to reduce variance.'). Toggle button label: 'Full Record ↓' when closed, 'Close ↑' when open. Do not modify `calculateHealthScore` logic — only the render layer." |
...
| I.6 | Forecast confidence bands + data quality indicator | ⬜ Not started | `index.html` (paste `ins-forecast` div) + `js/app.js` (paste `renderForecast` function) | "In js/app.js, update the `renderForecast()` cash flow projection chart to include confidence bands. For each future period data point, compute a ±1 standard deviation band from the last N periods (use all available periods, min 2). Add two additional datasets to the Chart.js config: `upperBand` (mean + stdDev) and `lowerBand` (mean - stdDev), both with `type:'line'`, `fill: false`, `borderDash:[4,4]`, `borderColor:'rgba(107,101,89,0.3)'`, `pointRadius:0`, `borderWidth:1`. Label them 'Optimistic' and 'Conservative' in the legend. In index.html, add a `<div id='ins-forecast-quality'>` row directly below the 3 summary stat chips (Avg Income / Avg Expenses / Avg Surplus). Content: a data quality indicator showing `N periods of data used`. If N < 3: amber pill 'Low confidence — {N} period{N!==1?\"s\":\"\"} of data. Forecasts improve after 3+.' using `background:rgba(184,87,43,0.1); color:var(--warn); border-radius:99px; padding:4px 10px; font-size:12px`. If N >= 3 and < 6: neutral grey pill 'Moderate confidence'. If N >= 6: green pill 'High confidence'. Compute N as `PAY_PERIOD.lastNPeriods(txs, 12).filter(p => p.length > 0).length`. Do not change the category forecast chart." |
...
| I.8 | Malaysia-specific context layer — local financial events | ⬜ Not started | `index.html` (paste `ins-spend` div) + `js/app.js` (paste `renderPeriodNarrative` function from I.3) | "In js/app.js, add a `getMalaysiaContext()` function that returns relevant local context for the current date. Define a hardcoded lookup of annual financial events in Malaysia by month: Jan → 'New Year spending spike typical'; Feb → 'Chinese New Year — expect 1.5–2× food and shopping spend'; Mar → null; Apr → 'School holidays — travel and entertainment may rise'; May → null; Jun → 'School holidays + mid-year sales'; Jul → null; Aug → 'Merdeka / National Day — sales period'; Sep → null; Oct → 'Deepavali — festive spending period'; Nov → 'Singles Day and year-end sales (Shopee, Lazada)'; Dec → 'Christmas + year-end bonus season — high spend month'. Also check if the current month matches Ramadan or Hari Raya dates for 2025–2027 (hardcode: Ramadan 2026 = Mar 1–Mar 30, Hari Raya 2026 = Mar 31; Ramadan 2027 = Feb 18–Mar 19, Hari Raya 2027 = Mar 20). If a context string exists for the current month, render a soft contextual note at the bottom of the `ins-narrative` card from I.3 (or create the card if I.3 is not yet built): `<p style='font-size:12px;color:var(--ink-4);margin-top:8px;border-top:1px solid var(--line);padding-top:8px'>📍 {contextString}</p>`. If no context applies, render nothing. This is display-only — no new Firestore reads or writes." |
