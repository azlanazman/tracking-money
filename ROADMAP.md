...
| # | Session Goal | Status | Files to bring | Prompt to use in Claude Code |
|---|---|---|---|---|
| 2.1 | Financial Health Score on dashboard | ⬜ Not started | `index.html` (paste `sc-dashboard` div + `renderDashboard` function + `renderBudgets` for budget data pattern) | "In index.html, add a Financial Health Score (0–100) card to the `sc-dashboard` screen. Calculate from four equally weighted signals: (1) budget compliance — spending stayed under total budget this period; (2) savings on track — at least one savings transaction this period; (3) fixed cost ratio — what % of 3-month avg income goes to fixed categories (Loan, Bills, Takaful, CC, Subs); (4) burn rate safety — will budget last the full period at current rate. Show the score as a large number, a colour-coded label (Healthy/Fair/At risk), and a one-line explanation of the biggest factor pulling it down. Recalculate inside `renderDashboard`." |
